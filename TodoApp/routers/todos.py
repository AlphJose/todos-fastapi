...
class Todo(BaseModel):
    ...
    priority: int = Field(gt=0, lt=6, description="The priority should be from 1-5")
    complete: bool
...
async def read_all_by_user(user: dict = Depends(get_current_user),
    ...
    return db.query(models.Todos).filter(models.Todos.owner_id == user_id).all()
